...
function post(kind: 'stdout' | 'stderr' | 'result' | 'error', text: string) { self.postMessage({ kind, text }) }
...
self.onmessage = async (e: MessageEvent<RunMessage>) => {
    ...
    try {
        transpiled = ts.transpileModule(code, { compilerOptions: { module: ts.ModuleKind.None, target: ts.ScriptTarget.ES2020 }, reportDiagnostics: false }).outputText;
    } catch (err) {
    ...
    try {
        ...
        const result = await run(sandboxesConsole);
        if (result !== undefined) post('result', String(result));
    } catch (err) { post('error', (err as Error).message) }
